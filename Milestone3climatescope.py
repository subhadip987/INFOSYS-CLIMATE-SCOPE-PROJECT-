...
# --------------------------------------------------
# COUNTRY CENTER COORDINATES (for map zoom)
# --------------------------------------------------
@st.cache_data
...
# --------------------------------------------------
# CHOROPLETH MAP VIEW  ← NOW SYNCED TO focused_country
# --------------------------------------------------
def choropleth_view(df, focused_country=None):
    ...
    focused_iso = None
    if focused_country and focused_country in iso_map:
        focused_iso = iso_map[focused_country]

    fig = go.Figure()

    # Base choropleth — all countries
    ...
    # Geo projection — zoom to selected country using orthographic rotation
    geo_kwargs = dict(
        showframe=False,
        showcoastlines=True,
    ...
    if col_toggle1.button("🌍 Reset to World View", key="reset_map"):
        st.session_state["map_override_country"] = None
        st.rerun()
    ...
    if focused_country:
        ...
        if not focused_val.empty:
            ...
            insight_card(
                ...
                f"Ranked <b>#{rank}</b> out of <b>{total}</b> countries in the dataset."
            )
...
def dashboard_basic():
    ...
    with col2: kpi_card("AVG WIND SPEED", f"{avg_wind:.1f}", "km/h  |  Max: " + f"{max_wind:.0f} km/h", "💨", "#7b2fff")
    ...
    insight_card("📋 CONDITION SUMMARY", "Predominant Weather Patterns",
    ...
    # ── Choropleth now synced to the selected country ──────────────────
    st.divider()
    choropleth_view(df, focused_country=selected_country)   # ← KEY CHANGE
...
def single_country_view(df, metric):
    ...
    with tab5:
        # Pro map tab — synced to selected country
        choropleth_view(df, focused_country=country)   # ← KEY CHANGE
...
def regional_comparison_view(df, metric):
    ...
    st.divider()
    # Focus on first selected country
...
def dashboard_pro():
    ...
    if mode == "Single Country":
        ...
        choropleth_view(df)  # Global map — no specific focus
