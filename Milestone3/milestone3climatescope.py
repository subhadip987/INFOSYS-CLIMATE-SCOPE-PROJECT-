...
# --------------------------------------------------
# COUNTRY ISO MAP HELPER (for choropleth)
# --------------------------------------------------
@st.cache_data
def get_country_iso_map():
    """Common country name → ISO3 mapping."""
    return {
...
def choropleth_view(df):
    ...
    if country_agg.empty:
        st.warning("Not enough country matches for ISO codes. Check country name spelling in your CSV.")
        return
    ...
    with col2:
        fig2 = px.bar(
            top10, x="Country", y=map_metric,
            color=map_metric,
            color_continuous_scale="Plasma",
            title=f"Top 10 Countries — {map_metric}",
        )
        apply_dark(fig2)
...
def auth_page():
    ...
    .stApp::after {
        ...
        background: url('https://images.unsplash.com/photo-1504608524841-42fe6f032b4b?w=1600') center/cover no-repeat;
        opacity: 0.08;
        z-index: -1;
    }
    </style>
    ...
    with col_m:
        st.markdown(f'<div style="text-align:right; margin-bottom:20px;"><span class="live-time">⏱ {now}</span></div>', unsafe_allow_html=True)
        st.markdown("""
        <div style="text-align:center; padding: 40px 0 24px;">
            ...
            <div style="color:#8892a4; font-size:14px; margin-top:6px; letter-spacing:2px; text-transform:uppercase;">
...
def dashboard_basic():
    ...
    selected_timer = st.sidebar.selectbox(
        "Refresh Interval",
    ...
    weather_images = [
        ...
        "https://images.unsplash.com/photo-1482192596544-9eb780fc7f66?w=1400",
        "https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=1400",
        "https://images.unsplash.com/photo-1505765050516-f72dcac9c60b?w=1400",
        "https://images.unsplash.com/photo-1502086223501-7ea6ecd79368?w=1400",
        "https://images.unsplash.com/photo-1500534314209-a25ddb2bd429?w=1400",
        "https://images.unsplash.com/photo-1499346030926-9a72daac6c63?w=1400",
        "https://images.unsplash.com/photo-1470770903676-69b98201ea1c?w=1400"
    ]
    ...
    st.image(
        ...
        caption=f"🌤️ Weather Scene #{idx+1}"
    )
    ...
    with col1:
        kpi_card("AVG TEMPERATURE", f"{avg_temp:.1f}°", f"Max {max_temp:.1f}° | Min {min_temp:.1f}°", "🌡️", "#00d4ff")
    with col2:
        kpi_card("AVG WIND SPEED", f"{avg_wind:.1f}", "km/h  |  Max: " + f"{max_wind:.0f} km/h", "💨", "#7b2fff")
    with col3:
        kpi_card("HUMIDITY", f"{avg_humidity:.0f}%", "Relative humidity average", "💧", "#06d6a0")
    with col4:
        kpi_card("PRESSURE", f"{avg_pressure:.0f}", "millibars (mb)", "🔵", "#ffd93d")

    col5, col6, col7, col8 = st.columns(4)
    with col5:
        kpi_card("TOTAL PRECIP", f"{total_precip:.1f}", "mm cumulative rainfall", "🌧️", "#ff6b6b")
    with col6:
        kpi_card("UV INDEX", f"{uv_index:.1f}", "Avg UV radiation level", "☀️", "#ffd93d")
    with col7:
        kpi_card("VISIBILITY", f"{visibility:.1f}", "km average visibility", "👁️", "#00d4ff")
    with col8:
        kpi_card("DATA POINTS", f"{data_points:,}", f"Records for {selected_country}", "📦", "#7b2fff")

    col9, col10, col11, col12 = st.columns(4)
    with col9:
        kpi_card("FEELS LIKE", f"{feels_like:.1f}°", "Apparent temperature avg", "🤔", "#06d6a0")
    with col10:
        kpi_card("DEW POINT", f"{dew_point:.1f}°", "Avg dew point Celsius", "🌫️", "#ff6b6b")
    with col11:
        kpi_card("CLOUD COVER", f"{cloud_cover:.0f}%", "Average cloud coverage", "☁️", "#8892a4")
    with col12:
        kpi_card("WIND GUSTS", f"{gust_kph:.1f}", "km/h average gusts", "🌪️", "#ffd93d")
    ...
    insight_card("💨 WIND & ATMOSPHERE", f"Atmospheric Dynamics",
        ...
        f"The pressure trend indicator {'shows variation' if 'pressure_trend' in country_df.columns else 'is stable'}.")

    insight_card("🌧️ PRECIPITATION & HUMIDITY", f"Moisture & Rainfall Analysis",
        ...
        f"Cloud cover sits at <b>{cloud_cover:.0f}%</b> on average. "
    ...
    insight_card("📋 CONDITION SUMMARY", f"Predominant Weather Patterns",
        f"The most frequent weather condition is <b>{common_cond}</b>. "
        f"Extreme weather events (storm/thunder/snow) account for "
        f"<b>{extreme_cond/max(data_points,1)*100:.1f}%</b> of observations. "
        f"Overall climate risk score: <b>{risk_score}/100</b> ({label_risk.split(' ',1)[1]}).")
    ...
    if "last_updated" in country_df.columns:
        ...
        with col2:
            if "season" in country_df.columns:
                ...
                fig2 = px.bar_polar(
                    season_df, r=temp_col, theta="season",
    ...
    if "wind_degree" in country_df.columns:
        ...
        with col1:
            fig_wind = go.Figure(go.Barpolar(
                ...
                marker_color=wind_rose['wind_kph'],
    ...
    with c1:
        kpi_card("HOTTEST DAY", f"{hot_day[temp_col]:.1f}°{unit[0]}",
                 str(hot_day.get('last_updated', 'N/A'))[:10], "🔥", "#ff6b6b")
    with c2:
        kpi_card("COLDEST DAY", f"{cold_day[temp_col]:.1f}°{unit[0]}",
                 str(cold_day.get('last_updated', 'N/A'))[:10], "🧊", "#00d4ff")
    with c3:
        temp_range = max_temp - min_temp
        kpi_card("TEMP RANGE", f"{temp_range:.1f}°{unit[0]}", "Max swing recorded", "📐", "#7b2fff")
...
def single_country_view(df, metric):
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 Distributions", "🍂 Seasonal", "🚨 Extremes", "📉 Decomposition", "🗺️ Global Map"])
...
def regional_comparison_view(df, metric):
    st.markdown(f'<div class="hero-banner"><div class="hero-title">Regional Comparison</div><div class="hero-sub">Side-by-side multi-country analysis</div></div>', unsafe_allow_html=True)
    ...
    c1, c2 = st.columns(2)
    with c1:
        fig_box = px.box(comp_df, x="country", y=metric, color="country", title="Statistical Range")
        apply_dark(fig_box)
    ...
    # Radar chart comparing countries
...
def similarity_view(df, metric):
    st.markdown(f'<div class="hero-banner"><div class="hero-title">Climate Similarity Index</div><div class="hero-sub">AI-powered climate matching between regions</div></div>', unsafe_allow_html=True)
    ...
    for idx, city in enumerate([city_a, city_b]):
        ...
        fig = px.histogram(cdf, x=metric, title=f"{city} — {metric} distribution",
                           color_discrete_sequence=['#00d4ff' if idx == 0 else '#7b2fff'])
        apply_dark(fig)
        [col1, col2][idx].plotly_chart(fig, use_container_width=True)

    # AI insight on similarity
...
def dashboard_pro():
    ...
    mode = st.sidebar.radio("Analysis Mode", ["Single Country", "Regional Comparison", "Similarity Index", "🗺️ Global Map"])
    metric = st.sidebar.selectbox("Select Metric", [c for c in df.select_dtypes(include=np.number).columns
                                                     if c not in ['month']])

    # Extra images in sidebar
    sidebar_imgs = [
